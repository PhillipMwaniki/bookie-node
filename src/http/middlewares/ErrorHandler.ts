...
export class ErrorHandler {
    ...
    static handleErrors(err: any, req: Request, res: Response, next: NextFunction) {
        ...
        if (err instanceof EntityNotFoundError) {
            return ResponseUtil.sendErrorResponse(res, "Item/page you are looking for does not exist", 404, null);
        }

        if (err.length > 0 && err[0] instanceof ValidationError) {
            const errors = ErrorHandler.formatErrors(err);
            return ResponseUtil.sendErrorResponse(res, "Invalid input", 422, errors);
        }
